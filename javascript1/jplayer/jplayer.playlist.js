...
_createListItem:function(a){
    var texto = a.title.split("~");////divido "a.title" para sacar el id y el titulo
    
    var c=this,d="<li id = 'audio_"+texto[0]+"' class='listaAudios'><div>";

    var _esPropietario = $.trim(texto[2]);

    if(_esPropietario == "verdadero"){//verifico si llega un valor en la posicion 2, que determina si es el propietario del audio y debe mostrarse el boton borrar
        d+= "<a href='javascript:;' class='"+this.options.playlistOptions.removeItemClass+
        "'><img class='mitadMargenSuperior' src='http://media.ablaonline.org/estilos/imagenes/eliminar2.png' ayuda='Click to delete this audio file'/></a>";//remplazo la x por una imagen de eliminar	
        
    }
    
	d+= "<div class='contenedorComentariosAudios'><div onclick='' ayuda='Write comment' propietarioitem='"+texto[3]+"' registro='"+texto[0]+"' modulo='AUDIOS' ruta='/ajax/users/comment' class=' contenedorBotonComentariosItems botonComentariosItemsGris' ";
	d+= "id='contenedorBotonComentariosItems'>  "+texto[4]+"  </div> </div>";    

    if(a.free){
        var e=!0;
        d+="<span class='"+this.options.playlistOptions.freeGroupClass+"'>(";
        b.each(a,function(a,f){
            b.jPlayer.prototype.format[a]&&(e?e=!1:d+=" | ",d+="<a class='"+c.options.playlistOptions.freeItemClass+"' href='"+f+"' tabindex='1' class=''>"+a+"</a>")
            });
        d+=")</span>"
        }
        
        //agrego antes del "a" el campo oculto donde voy a guardar el id de la cancion        
        d+="<a href='javascript:;' class='"+this.options.playlistOptions.itemClass+" itemAudio' tabindex='1' onclick=' var item = $(this); activarFuncionesComentar(item);'><input type='hidden' id='id' value='"+texto[0]+"'/>"+texto[1]+(a.artist?" <span class='jp-artist'>by "+a.artist+"</span>":"")+"</a>";
    d+="</div></li>";
    return d
    },
_createItemHandlers:function(){
    ...
    b(a.cssSelector.playlist+" a."+this.options.playlistOptions.removeItemClass).die("click").live("click",
        function(){
            var idAudio = $(this).parents("li").find("#id").val();//captura el texto del li
            $(this).parents("li").attr("id", "audio_"+idAudio);//le agrego el id al li segun la cancion
            eliminarAudio(idAudio);//llamo a la funcion para eliminar un audio
